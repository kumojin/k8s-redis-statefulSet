...
if  [[ ! -f /data/sentinel.conf ]]; then
  while true; do
    ...
    master=$(timeout 1 redis-cli -h ${REDIS_SENTINEL} -p ${REDIS_SENTINEL_PORT} --csv SENTINEL get-master-addr-by-name mymaster | tr ',' ' ' | cut -d' ' -f1)
    if [[ -n "${master}" ]]; then
    ...
    if [[ -n "$master" ]]; then
      ...
      echo "sentinel down-after-milliseconds mymaster 1000" >> /data/sentinel.conf
